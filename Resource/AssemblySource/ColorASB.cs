using System;
using System.Windows;

namespace Resource.AssemblySource
{
   public class ColorASB
    {
        public static ComponentResourceKey Red
        {
            get => new ComponentResourceKey(typeof(ColorASB),"red");
        }

        public static ComponentResourceKey Green
        {
            get => new ComponentResourceKey(typeof(ColorASB),"green");
        }

        public static ComponentResourceKey Orange
        {
            get => new ComponentResourceKey(typeof(ColorASB),"orange");
        }

        public static ComponentResourceKey BD200
        {
            get => new ComponentResourceKey(typeof(ColorASB),"bd200");
        }

        public static ComponentResourceKey BD300
        {
            get => new ComponentResourceKey(typeof(ColorASB),"bd300");
        }
        public static ComponentResourceKey BD400
        {
            get => new ComponentResourceKey(typeof(ColorASB),"bd400");
        }
        public static ComponentResourceKey BD500
        {
            get => new ComponentResourceKey(typeof(ColorASB),"bd500");
        }
        public static ComponentResourceKey BD600
        {
            get => new ComponentResourceKey(typeof(ColorASB),"bd600");
        }
        public static ComponentResourceKey BD700
        {
            get => new ComponentResourceKey(typeof(ColorASB),"bd700");
        }
        public static ComponentResourceKey NL700
        {
            get => new ComponentResourceKey(typeof(ColorASB), "nl700");
        }

        public static ComponentResourceKey Grey200
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey200");
        }
        public static ComponentResourceKey Grey300
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey300");
        }
        public static ComponentResourceKey Grey100
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey100");
        }
        public static ComponentResourceKey Grey400
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey400");
        }

        public static ComponentResourceKey Grey100b
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey100b");
        }

        public static ComponentResourceKey Grey50
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey50");
        }

        public static ComponentResourceKey Grey500
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey500");
        }

        public static ComponentResourceKey Grey600
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey600");
        }
        public static ComponentResourceKey Grey700
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey700");
        }
        public static ComponentResourceKey Grey800
        {
            get => new ComponentResourceKey(typeof(ColorASB), "grey800");
        }

    }
}
